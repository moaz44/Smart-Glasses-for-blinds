...
#
# # Convert the model
# converter = tf.lite.TFLiteConverter.from_saved_model('venv') # path to the SavedModel directory
        ...
        while True:
            if stop:
                ...
                if detections['detection_scores'][0] >= .5:
                    list.insert(len(list), self.category_index[detections['detection_classes'][0] + 1]['name'])
                print("Start")
                list.sort()
                listt.sort()
                if listt != list:
